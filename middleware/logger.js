...
const logger = (req, res, next) => {
    ...
    logEvents(`<span class="math-inline">\{req\.method\}\\t</span>{clientIP}\t${userAgent}\t${req.url}\t${JSON.stringify(req.headers)}`, `reqLog.txt`);
...
const unauthorizedAccessAttempt = (req) => {
    const clientIP = req.headers['x-forwarded-for'] || req.connection.remoteAddress;
    const userAgent = req.headers['user-agent'];
    logEvents(`UNAUTHORIZED ATTEMPT\t${clientIP}\t${userAgent}\t${req.url}`, `unauthlog.txt`);
}



module.exports = { logger, logEvents, errorHandler, unauthorizedAccessAttempt };
